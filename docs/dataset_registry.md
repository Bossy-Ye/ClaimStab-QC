...
```bash
PYTHONPATH=. ./venv/bin/python -m claimstab.pipelines.claim_stability_app --task maxcut --suite core --space-presets compilation_only,sampling_only,combined_light --sampling-mode random_k --sample-size 16 --sample-seed 7 --backend-engine basic --out-dir output/presentation/main/maxcut_core
```
...
```bash
PYTHONPATH=. ./venv/bin/python -m claimstab.pipelines.claim_stability_app --task bv --suite core --space-presets compilation_only,sampling_only,combined_light --sampling-mode random_k --sample-size 16 --sample-seed 7 --backend-engine basic --out-dir output/presentation/main/bv_core
```
